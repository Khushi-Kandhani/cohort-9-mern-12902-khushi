...
describe("Notes Controller", () => {
  const userA: TestUser = { name: "User A", email: "usera@example.com", password: "test1234" };
  const userB: TestUser = { name: "User B", email: "userb@example.com", password: "test1234" };

  async function registerAndLogin(user: TestUser): Promise<string> {
  ...
  describe("POST /api/notes", () => {
    ...
    it("persists the category field", async () => {
      const token = await registerAndLogin(userA);

      const res = await request(app)
        .post("/api/notes")
        .set("Authorization", `Bearer ${token}`)
        .send({ title: "Categorized note", content: "content", category: "Work" });
